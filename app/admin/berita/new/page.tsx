...
import { createBeritaAction } from "@/app/actions/berita";
...
export default function CreateBeritaPage() {
  ...
  const handlePublish = async () => {
    ...
    if (result.success) {
      setSaveMessage("Berita telah berhasil diterbitkan!");
      setTimeout(() => {
        router.push("/admin/berita");
      }, 1000);
    } else {
    ...
    <div className="h-screen bg-background text-foreground flex flex-col font-sans overflow-hidden">
      ...
      <header className="sticky top-0 z-40 flex h-16 shrink-0 items-center justify-between border-b border-border bg-background/80 backdrop-blur-md px-4 md:px-6 transition-all">
        <div className="flex items-center gap-3 ">
          ...
          >
            <Link href="/admin/berita" className="flex items-center gap-2">
                  ...
                  {coverSrc ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-12 px-6 text-center">
                      ...
                      <h3 className="text-sm font-semibold text-foreground">
                        {isUploadingCover ? "Mengunggah Gambar Cover..." : "Unggah Gambar Cover"}
                      </h3>
                      <p className="mt-1 text-xs text-muted-foreground">
                        Tarik gambar atau klik di sini (PNG, JPG, WebP — maks. 5
                        MB)
                      </p>
                    </div>
                  )}
                ...
                <div className="space-y-5">
                  ...
                  <div className="rounded-xl border border-border bg-muted/30 p-3 space-y-2">
                    ...
                    <div className="flex justify-between items-center text-xs">
                      ...
                      <span className="font-semibold text-foreground">
                        ~{Math.max(1, Math.ceil(blocks.length * 0.8))} min baca
                      </span>
                    </div>
                  </div>
                </div>
      ...
      {block.type === "image" && (
        <div className="space-y-2">
          <div
            ...
            className={cn(
              "group/img relative w-full cursor-pointer overflow-hidden rounded-xl border-2 border-dashed transition-all",
              block.imageSrc
                ? "border-transparent bg-muted/20"
                : "border-border bg-card hover:border-primary/40 p-6 flex flex-col items-center justify-center gap-2 text-muted-foreground",
            )}
          >
            {block.imageSrc ? (
