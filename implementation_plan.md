...
Implementasi sistem pengurusan konten (CRUD) untuk **Jejak Karya** (Project/Portofolio Siswa) di SMK TI BAZMA. Workflow UI admin akan mengikuti standar dan layout yang ada pada **CRUD Berita** (`app/admin/berita`), namun disesuaikan untuk bidang-bidang khusus portofolio (kategori Website, Design, Video, IoT, tantangan, pendekatan, hasil, tim/penulis, tautan demo, serta galeri karya) dan dilengkapi **Preview Mode** khas tampilan publik Jejak Karya.
...
> [!IMPORTANT]
> **Data Migration Note**:
> 1. Data dummy awal pada `lib/jejak-karya-data.ts` akan dimigrasikan ke database PostgreSQL via Prisma / Seed agar data bersifat dinamis dan konsisten.
> 2. Skema Prisma baru `JejakKarya` akan ditambahkan dan disinkronkan menggunakan Server Actions.
