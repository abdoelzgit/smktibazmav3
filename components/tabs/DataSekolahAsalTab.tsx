...
export default function DataSekolahAsalTab() {
  ...
  const handleSubmit = async (formData: FormData) => {
    setStatus("Menyimpan...");
    const result = await saveDataSekolahAction(formData);
    setStatus(result.success ? "Data sekolah berhasil disimpan." : result.error ?? "Data sekolah gagal disimpan.");
  };
    ...
    <form action={handleSubmit} className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
        ...
        <div>
          ...
          <a href="https://referensi.data.kemdikbud.go.id/index11.php" target="_blank" rel="noopener noreferrer" className="mb-1 block text-sm text-blue-500 underline">
        ...
        <div>
          ...
          <select name="statusSekolah" className="w-full border border-gray-400 rounded-lg px-3 py-2 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white">
